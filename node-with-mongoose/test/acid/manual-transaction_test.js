import { Account, Transaction, ManualTransactionExample } from '../../src/acid/index.js';
...
describe('Manual Transaction Management', () => {
    ...
    describe('Successful Transactions', () => {
        it('should transfer money successfully and commit transaction', async () => {
            const result = await ManualTransactionExample.transferMoney('ACC001', 'ACC002', 200);
        ...
        it('should handle multiple operations in single transaction', async () => {
            ...
            const result = await ManualTransactionExample.transferMoney('ACC001', 'ACC002', 100);
    ...
    describe('Failed Transactions and Rollback', () => {
        it('should rollback transaction when error occurs', async () => {
            try {
                await ManualTransactionExample.transferMoneyWithError('ACC001', 'ACC002', 200);
        ...
        it('should rollback when insufficient funds', async () => {
            try {
                await ManualTransactionExample.transferMoney('ACC001', 'ACC002', 1500);
                expect.fail('Should have thrown an error');
            } catch (error) {
        ...
        it('should rollback when from account not found', async () => {
            try {
                await ManualTransactionExample.transferMoney('INVALID', 'ACC002', 200);
                expect.fail('Should have thrown an error');
            } catch (error) {
        ...
        it('should rollback when to account not found', async () => {
            try {
                await ManualTransactionExample.transferMoney('ACC001', 'INVALID', 200);
                expect.fail('Should have thrown an error');
            } catch (error) {
    ...
    describe('Concurrent Transactions', () => {
        it('should handle concurrent transfers correctly', async () => {
            ...
            const transfer1 = ManualTransactionExample.transferMoney('ACC001', 'ACC002', 100);
            const transfer2 = ManualTransactionExample.transferMoney('ACC001', 'ACC003', 150);
